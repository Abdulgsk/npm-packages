...
console.log(chalk.green.bold('\nWelcome to create-express-backend 🚀\n'));

async function init() {
  const { projectName, mongoURI, useCors, useNodemon, port } = await inquirer.prompt([
  {
    type: 'input',
    name: 'projectName',
...
import connectDB from './db/db.js';${useCors ? "\nimport cors from 'cors';" : ''}
import mongoose from 'mongoose';
...
export default router;`
  ...
  execSync(`npm install ${dependencies.join(' ')}`, { cwd: targetDir, stdio: 'inherit' });

  if (devDependencies.length > 0) {
    console.log(chalk.blue('\nInstalling dev dependencies...'));
    execSync(`npm install -D ${devDependencies.join(' ')}`, { cwd: targetDir, stdio: 'inherit' });
  }
